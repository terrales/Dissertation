...
_ROUGE_PATH = '/home/terra/DYLE/dyle/lib/python3.9/site-packages/pyrouge/tools/ROUGE-1.5.5'
rouge_diff_thresh = 0
...
mode = "test"
# Change to own path
path = '/home/terra/DYLE/data/AVENI/data/jsonl/{}.jsonl'.format(mode)
data = []
with open(path) as f:
    for line in f:
...
def extract_oracle(samples):
    ...
    for idx, data_entity in samples:
        ...
        # creating dir because i was getting errors
        ref_dir = './greedy/ref_{}/'.format(mode)
        os.makedirs(ref_dir, exist_ok=True)
        # Obtain oracle index
        with open(os.path.join(ref_dir, '{}.ref'.format(idx)), 'w') as f:
            for sent in sent_tokenize(ref):
                print(sent, file=f)
        # creating dir because i was getting errors
        dec_dir = './greedy/dec_{}/'.format(mode)
        os.makedirs(dec_dir, exist_ok=True)
        # Obtain gold summary reference
        with open(os.path.join(dec_dir, '{}.dec'.format(idx)), 'w') as f:
            for sent in sent_tokenize(oracle):
                print(sent, file=f)
        # creating dir because i was getting errors
        index_dir = './greedy/index_{}/'.format(mode)

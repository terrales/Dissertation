...
from dataloaders.aveni import AVENI
from dataloaders.product import Product
from dataloaders.academic import Academic
from dataloaders.committee import Committee
...
class Experiment(object):
    ...
    def __init__(self, load_train=True):
        ...
        elif config.target_task in ['ac-aveni-latent',
                                  ]:
            if load_train:
                self.train_set = Academic('train', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.val_set = Academic('valid', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.test_set = AVENI('test', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
        elif config.target_task in ['comm-aveni-latent',
                                  ]:
            if load_train:
                self.train_set = Committee('train', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.val_set = Committee('valid', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.test_set = AVENI('test', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
        elif config.target_task in ['prod-aveni-latent',
                                  ]:
            if load_train:
                self.train_set = Product('train', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.val_set = Product('valid', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.test_set = AVENI('test', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
        elif config.target_task in ['all-aveni-latent',
                                  ]:
            if load_train:
                self.train_set = QMSum('train', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.val_set = QMSum('valid', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
        ...
        elif config.target_task in ['product-latent',
                                  ]:
            if load_train:
                self.train_set = Product('train', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.val_set = Product('valid', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.test_set = Product('test', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
        elif config.target_task in ['ac-prod-latent',
                                  ]:
            if load_train:
                self.train_set = Academic('train', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.val_set = Academic('valid', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.test_set = Product('test', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
        elif config.target_task in ['comm-prod-latent',
                                  ]:
            if load_train:
                self.train_set = Committee('train', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
            self.val_set = Committee('valid', retriever_tokenizer=self.retriever_tokenizer, generator_tokenizer=self.generator_tokenizer)
...
#            print('iter',self.iter_num)
    ...
    def seq_evaluate_gen(self, test, beam_size):
     ...
     #       print(torch.cuda.memory_summary(device=None, abbreviated=False))
            ...
            with torch.no_grad():
                ...
                if config.loss_alpha != 0:
                    outputs = self.generator.generate(context_input_ids=context_input_ids[:, retriever_topk_indices].contiguous().view(context_input_ids.shape[0] * config.top_k, -1),
                                                      ...
                                                      # stopping_criteria=StoppingCriteriaList(MaxLengthCriteria(max_length=config.max_target_len))
        ...
        if print_res:
            peep_num = 3
            for sent_id in range(predictions):
                print('Pred:\n{}'.format(predictions[sent_id]))
                print('-' * 20)
                print('topk:\n{}'.format(topks[sent_id]))
                print('-' * 20)
